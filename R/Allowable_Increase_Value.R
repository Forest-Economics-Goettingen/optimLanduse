...
for (i in unique(dat$landUse)) {
  for (j in unique(dat$indicator)) {
    ...
    if (z >= 50) {
      AllowableIncreaseValue[nrow(AllowableIncreaseValue) + 1, ] <- c(paste0(i," | ", j), 5000, round((dat$indicatorValue[dat$landUse == i & dat$indicator == j]*50), digits = 2))
      ...
      AllowableIncreaseValue[nrow(AllowableIncreaseValue) + 1, ] <- c(paste0(i," | ", j), (z - 0.001)*100, round((dat$indicatorValue[dat$landUse == i & dat$indicator == j]*(z-0.001)), digits = 2))}
  }}

if (any(AllowableIncreaseValue$`Allowable Increase [%]` == 5000)) {
  print(paste0("Warning: Maximum increase of 5000 % is reached ", dim(AllowableIncreaseValue[AllowableIncreaseValue$`Allowable Increase [%]` == 5000,])[1], " times."))
}
