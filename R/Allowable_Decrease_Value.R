...
for (i in unique(dat$landUse)) {
  for (j in unique(dat$indicator)) {
    ...
    if (z >= 50) {
      AllowableDecreaseValue[nrow(AllowableDecreaseValue) + 1, ] <- c(paste0(i," | ", j), 5000, round((dat$indicatorValue[dat$landUse == i & dat$indicator == j]*50), digits = 3))
      ...
      AllowableDecreaseValue[nrow(AllowableDecreaseValue) + 1, ] <- c(paste0(i," | ", j), (z - 0.001)*100, round((dat$indicatorValue[dat$landUse == i & dat$indicator == j]*(z-0.001)), digits = 3))}
  }}

if (any(AllowableDecreaseValue$`Allowable Decrease [%]` == 5000)) {
  print(paste0("Warning: Maximum decrease of 5000 % is reached ", dim(AllowableDecreaseValue[AllowableDecreaseValue$`Allowable Decrease [%]` == 5000,])[1], " times."))
}
